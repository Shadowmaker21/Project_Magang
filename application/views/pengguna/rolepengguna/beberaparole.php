<div class='row'>
	<div class="col-sm-4">	
		<h4>Tindakan</h4>
		<?php if($hasil){ ?>
			<h5>Saya akan menghapus data tahun dengan daftar terlampir</h5>
	...
	<div class='col-sm-8 gr'>
		...
		<table class='table table-striped table-hover table-bordered'>
			...
			<?php 
				...
				if(!$hasil){ ?>
				<td colspan='3'>
					<center> Tidak ada role terpilih.</center>
				</td>
				<?php }
				foreach ($hasil as $data) { ?>
					<td><center><?php echo $i; ?></center></td>
					<td><?php echo ucwords($data->upriv_name) ?></td>
				</tr>
				<?php $i++; } ?>
			</tbody>
		</table>
...
<script type="text/javascript">
	...
	function hapus(){
		...
		var data = $("#id").val();
		data = {data:data};
		submit_ajax_function('POST','<?php echo base_url('pengguna/hapusselectedrole')?>',data,'json','#dataperusahaan',33);
	}
</script>
