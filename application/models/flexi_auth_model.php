...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	 ...
	 * @author Rob Hussey
	 */
	public function insert_user($email, $username, $bidang = FALSE, $password, $custom_data = FALSE, $group_id = FALSE)
	{
	    ...
	    $sql_insert = array(
			...
			$this->auth->tbl_col_user_account['bidang_id'] => $bidang,
	...
	public function namecompany_available($name = FALSE, $user_id = FALSE)
	{
	    if (empty($name))
	    {
			return FALSE;
	    ...
	    return $this->db->where($this->auth->tbl_col_perusahaan['nama_perusahaan'], $name)
			->count_all_results($this->auth->tbl_perusahaan) == 0;
	}
	...
	public function reset_pass($identity,$new_password,$salt){
		// Create hash of password and store.
		$hash_new_password = $this->generate_hash_token($new_password, $salt, TRUE);
		$data = array(
			'uacc_password' => $hash_new_password
		);
		$this->db->where('uacc_id',$identity);
