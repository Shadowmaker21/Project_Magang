...
class M_galeri extends CI_Model
{
	...
	public function galeri_save($user){
	...
	public function galeri_update($user){
		$album = $this->input->post('album');
		$judul = $this->input->post('judul');
		$deskripsi = $this->input->post('deskripsi');
		$date = date('Y-m-d H:i:s',time()+21600);
	    ...
	    $where = array('id' => $id);
	    $a = $this->get_table_row($where,'dtgaleri');
	...
	public function galeri_delete(){
		$id = $this->input->post('id');		
		$where = array('id' => $id);
		$a = $this->get_table_row($where,'galeri');
		$path = $a->file_path.$a->file_name;
		@unlink($path);
		if($this->db->delete('galeri',array('id' => $a->id))){
			$pesan = array(
				'jenis' => 'success',
				'title' => 'Tindakan Berhasil Dilakukan',
